...
function calServiceFee()
{
	...
	$.ajax({
		...
		success: function(result) {
			...
			if (fc_fee > 0) {
				$(".airport-fast-track-t").html(fc_fee + " USD x " + $(".group-size :selected").text() + " = " + (fc_fee*group_size) + " USD");
			} else {
				$(".airport-fast-track-t").html("0 USD");
			}
			...
			if (car_fee > 0) {
				$(".car-pickup-t").html(car_type + ", " + num_seat + " seats = " + car_fee + " USD");
			} else {
				$(".car-pickup-t").html("0 USD");
			}
